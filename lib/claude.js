...
async function claudeJSON({ system, prompt, maxTokens = 1500 }) {
  const { text, stopReason } = await callClaude({ system, prompt, maxTokens });
  ...
  try {
    ...
    throw new Error(`${parseErr.message} | stop_reason=${stopReason} | textLen=${text.length} | tail="${jsonSlice.slice(-80)}"`);
  }
}

// Invia un prompt a Claude e ne restituisce il testo grezzo (nessun parsing JSON) — usato per
// compiti semplici come la traduzione di un singolo paragrafo.
async function claudeText({ system, prompt, maxTokens = 700 }) {
  const { text } = await callClaude({ system, prompt, maxTokens });
  return text.trim();
}

module.exports = { claudeJSON, claudeText, MODEL };
